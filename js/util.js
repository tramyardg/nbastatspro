function CommonUtil() {
    ...
    this.alertMessage = function (alertType, yourMessage) {
        var h = '';
        h += '<div class="alert alert-' + alertType + '" role="alert">';
        h += yourMessage;
        h += '</div>';
        return h;
    };
    ...
    this.showHintFirstName = function (input) {
        $.ajax({
            url: "../nbastatspro/model/gethint.php",
    ...
    // Source: https://stackoverflow.com/users/1438628/majid-laissi
    // https://stackoverflow.com/questions/14446447/javascript-read-local-text-file
    this.readTextFile = function (file) {
        var result = null;
        var rawFile = new XMLHttpRequest(); // variable for making a request
    ...
    this.readTextFile1 = function (filePath) {
        $.ajax({
            url: filePath,
            async: false,
            success: function (data) {
                var pe = new CommonUtil();
                pe.pageExecute.fileContents = data;
            }
        });
    };
    ...
    this.statsHeader = {
        ...
        RANK: 1,
        PLAYER: 2,
        TEAM: 3, GP: 4, MIN: 5, FGM: 6, FGA: 7, FG_PCT: 8, FG3M: 9,
        FG3A: 10, FG3_PCT: 11, FTM: 12, FTA: 13, FT_PCT: 14, OREB: 15, DREB: 16, REB: 17,
        AST: 18, STL: 19, BLK: 20, TOV: 21, PF: 22, PTS: 23, EFF: 24, AST_TOV: 25, STL_TOV: 26
